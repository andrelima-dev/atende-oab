      ...
      {[...Array(5)].map((_, index) => (
        <Star
          ...
          className={`h-4 w-4 transition-colors ${
            index < Math.round(rating) ? "text-amber-400 fill-amber-400" : "text-slate-300"
          }`}
        />
      ))}
      ...
      <CardHeader className="bg-gradient-to-r from-slate-50 to-blue-50 dark:from-slate-800/50 dark:to-blue-900/30 pb-6 border-b border-slate-100 dark:border-slate-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{evaluations.length} avaliações</p>
          </div>
        </div>
      </CardHeader>
          ...
          {evaluations.length > 0 ? (
            evaluations.map((evaluation, index) => (
              <React.Fragment key={evaluation.id}>
                <div className={`p-4 rounded-xl border-2 transition-all hover:shadow-md ${getRatingColor(evaluation.rating)}`}>
